...
export default function LoomisOverlay({ canonId, view }: { canonId: string; view: View }) {
  ...
  return (
    <svg
      ...
      aria-hidden
    >
      <g fill="none" stroke={stroke} strokeWidth={Math.max(1, w / 260)} opacity={0.85}>
        {/* Plomada (aplomo) */}
        <line x1={cx} y1={0} x2={cx} y2={h} strokeDasharray={`${w / 90} ${w / 90}`} />
        {/* Bola craneal */}
        <circle cx={cx} cy={ballCy} r={ballR} />
        {/* Mandíbula → barbilla */}
        <line x1={cx - jawHalf} y1={browY} x2={cx} y2={chinY} />
        <line x1={cx + jawHalf} y1={browY} x2={cx} y2={chinY} />
        {/* Tercios faciales (ceja / nariz) + barbilla */}
        <line x1={cx - jawHalf} y1={browY} x2={cx + jawHalf} y2={browY} />
        <line x1={cx - jawHalf * 0.7} y1={noseY} x2={cx + jawHalf * 0.7} y2={noseY} />
        <line x1={0} y1={chinY} x2={w} y2={chinY} strokeDasharray={`${w / 120} ${w / 120}`} opacity={0.5} />
      </g>
    </svg>
  )
}
