  ...
  return (
    <div className="bg-[var(--color-surface-container)] border-t border-[var(--color-outline-variant)] shrink-0 px-4 py-1.5 flex items-center gap-4 overflow-x-auto whitespace-nowrap font-mono text-[10px] text-[var(--color-on-surface-variant)]">
      <span className="shrink-0">
        1 cuadro = <span className="text-[var(--color-primary)]">{round1(squareCm)} cm</span>
      </span>
      <span className="opacity-40">·</span>
      <span className="shrink-0">{objectCount} objeto{objectCount === 1 ? '' : 's'}</span>
      {selectedCount > 1 && (
        <>
          <span className="opacity-40">·</span>
          <span className="shrink-0 text-[var(--color-primary)]">{selectedCount} seleccionados</span>
        </>
      )}
      ...
      {!readOnly && selectedObj && (
        <div className="flex items-center gap-2.5 shrink-0">
          {field('X', cmOf(selectedObj.x), (cm) => onPatch({ x: pxOf(cm) }))}
          {field('Y', cmOf(selectedObj.y), (cm) => onPatch({ y: pxOf(cm) }))}
          {field('An', cmOf(selectedObj.w), (cm) => onPatch({ w: Math.max(0, pxOf(cm)) }))}
          {field('Al', cmOf(selectedObj.h), (cm) => onPatch({ h: Math.max(0, pxOf(cm)) }))}
          {field('∠', selectedObj.rotation || 0, (deg) => onPatch({ rotation: deg }), '°')}
        </div>
      )}
    </div>
  )
