...
function AppShellInner({ children }: { children: ReactNode }) {
  const { sidebarOpen, navbarOpen, toggleNavbar, edgeReveal, isBoards } = useChrome()
  ...
  // Con el navbar abierto, el toggle (cerrar) siempre se ve.
  const reopenRevealed = navbarOpen || !isBoards || edgeReveal.top || focused || hovered || !canHover
  ...
  return (
    <div className="min-h-screen bg-[var(--color-surface-container-lowest)] text-[var(--color-on-surface)] flex flex-col md:flex-row relative">
      ...
      {/* Toggle ÚNICO del navbar (mostrar/ocultar) — siempre en el mismo lugar
          (arriba, centrado): cierra cuando está abierto, abre cuando oculto. */}
      <motion.button
        type="button"
        ...
        aria-label={navbarOpen ? t('common.hideTopBar') : t('common.showTopBar')}
        title={navbarOpen ? t('common.hideTopBar') : t('common.showTopBar')}
        aria-pressed={navbarOpen}
        ...
        whileTap={{ scale: 0.95 }}
      >
        <span className="material-symbols-outlined text-[20px]" aria-hidden>{navbarOpen ? 'expand_less' : 'expand_more'}</span>
      </motion.button>
