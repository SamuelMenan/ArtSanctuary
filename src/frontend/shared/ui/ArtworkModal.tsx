  ...
  const modalContent = (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-6 bg-black/90 backdrop-blur-sm animate-in fade-in duration-300">
      ...
      <div className="bg-[var(--color-surface-container-lowest)] w-full h-full md:rounded-sm md:h-[90vh] md:max-w-7xl flex flex-col md:flex-row overflow-hidden relative shadow-2xl ring-1 ring-[var(--color-outline-variant)]">
        ...
        <button type="button" onClick={onClose} className="absolute top-4 right-4 z-50 bg-neutral-950/50 text-white rounded-full p-2 hover:bg-neutral-950 transition-colors">
          <span className="material-symbols-outlined text-2xl">close</span>
        </button>
        {onPrev && (
          <button type="button" onClick={onPrev} className="absolute left-4 top-1/2 -translate-y-1/2 z-50 bg-neutral-950/50 text-white rounded-full p-2 hover:bg-neutral-950 transition-colors hidden md:block">
            <span className="material-symbols-outlined">chevron_left</span>
          </button>
        )}
        {onNext && (
          <button type="button" onClick={onNext} className="absolute right-[31%] top-1/2 -translate-y-1/2 z-50 bg-neutral-950/50 text-white rounded-full p-2 hover:bg-neutral-950 transition-colors hidden md:block">
